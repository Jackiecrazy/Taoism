...
public class TaoBow extends ItemBow implements IAmModular, IElemental {
	public static ItemStack createRandomBow(EntityPlayer p, Random r) {
		...
		int wsw = r.nextInt(TaoConfigs.bowc.getType(TaoConfigs.bowc.name()[0]).size());
		String s = StaticRefs.STAVE;
		((TaoBow) ret.getItem()).setPart(s, ret, new PartData(s, MaterialsConfig.getRandomMat(r, AbstractWeaponConfigOverlord.lookup(s, wsw).matType()), wsw));
		wsw = r.nextInt(TaoConfigs.bowc.getType(TaoConfigs.bowc.name()[1]).size());
		s = StaticRefs.STRING;
		((TaoBow) ret.getItem()).setPart(s, ret, new PartData(s, MaterialsConfig.getRandomMat(r, AbstractWeaponConfigOverlord.lookup(s, wsw).matType()), wsw));
		wsw = r.nextInt(TaoConfigs.bowc.getType(TaoConfigs.bowc.name()[2]).size());
		s = StaticRefs.SIYAH;
		((TaoBow) ret.getItem()).setPart(s, ret, new PartData(s, MaterialsConfig.getRandomMat(r, AbstractWeaponConfigOverlord.lookup(s, wsw).matType()), wsw));
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
		...
		for (PartData a : getParts(stack).values()) {//so this is now an empty list eh
			if (a != null && a.getMat() != null) tooltip.add(a.getMat() + " " + AbstractWeaponConfigOverlord.lookup(a.getPart(), a.getOrdinal()).getName());
			/*else {
	...
	private float arrspd(ItemStack is) {
		...
		for (String s : getPartNames(is)) {
			if (getPart(is, s) != null) {
				...
				if (AbstractWeaponConfigOverlord.lookup(pd.getPart(), pd.getOrdinal()) == null || MaterialsConfig.findMat(pd.getMat()) == null) {
					System.out.println(AbstractWeaponConfigOverlord.lookup(pd.getPart(),pd.getOrdinal()));
					//System.out.println(MaterialsConfig.findMat(pd.getMat()));

					continue;
				}
				ret += AbstractWeaponConfigOverlord.lookup(pd.getPart(), pd.getOrdinal()).getDamageMultiplier() * MaterialsConfig.findMat(pd.getMat()).msw.arrowSpeed;
				numofparts++;
	...
	private float[] deriveStats(ItemStack is) {
		...
		for (String s : getPartNames(is)) {
			if (getPart(is, s) != null) {
				...
				//ran
				ret[3] += AbstractWeaponConfigOverlord.lookup(pd.getPart(), pd.getOrdinal()).getRange();
				//kin moku sui hi do
				ret[4] += AbstractWeaponConfigOverlord.lookup(pd.getPart(), pd.getOrdinal()).getElementalMultiplier() * MaterialsConfig.findMat(pd.getMat()).msw.affinityMetal;
				ret[5] += AbstractWeaponConfigOverlord.lookup(pd.getPart(), pd.getOrdinal()).getElementalMultiplier() * MaterialsConfig.findMat(pd.getMat()).msw.affinityWood;
				ret[6] += AbstractWeaponConfigOverlord.lookup(pd.getPart(), pd.getOrdinal()).getElementalMultiplier() * MaterialsConfig.findMat(pd.getMat()).msw.affinityWater;
				ret[7] += AbstractWeaponConfigOverlord.lookup(pd.getPart(), pd.getOrdinal()).getElementalMultiplier() * MaterialsConfig.findMat(pd.getMat()).msw.affinityFire;
				ret[8] += AbstractWeaponConfigOverlord.lookup(pd.getPart(), pd.getOrdinal()).getElementalMultiplier() * MaterialsConfig.findMat(pd.getMat()).msw.affinityEarth;
