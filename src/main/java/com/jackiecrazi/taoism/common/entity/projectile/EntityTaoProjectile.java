...
public class EntityTaoProjectile extends EntityArrow implements IDamageType {
    ...
    private static final Field
            ...
            fire = ObfuscationReflectionHelper.findField(Entity.class, "field_190534_ay");
    ...
    protected void onHitEntity(Entity hit) {
        if (this.isBurning()) hit.setFire(getFire());
    }
    ...
    public void shoot(Vec3d vec, float velocity, float inaccuracy) {
        inGround=false;
        shoot(vec.x, vec.y, vec.z, velocity, inaccuracy);
        markVelocityChanged();
        sync();
    }

    public void shootTo(Vec3d vec, float velocity, float inaccuracy) {
        inGround=false;
        if(Double.isNaN(vec.x)||Double.isNaN(vec.y)||Double.isNaN(vec.z))return;
        shoot(vec.x - posX, vec.y - posY, vec.z - posZ, velocity, inaccuracy);
        markVelocityChanged();
        sync();
    }

    public void shootTo(BlockPos vec, float velocity, float inaccuracy) {
        inGround=false;
        shoot(vec.getX() - posX, vec.getY() - posY, vec.getZ() - posZ, velocity, inaccuracy);
        markVelocityChanged();
        sync();
    }
